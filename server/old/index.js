...
const fileSaver = require("../utils/wav");
...
const server = net.createServer((socket) => {
    ...
    socket.on("data", (lpcmChunk) => {
        ...
        if (receiverBytes == 32000 * 3) {
            ...
            fileSaver.makeOutputFolder("output", () => {
                fileSaver.save(lpcmBuffer, "output/" + fileName);
                // fileSaver.saveFrames(lpcmBuffer, fileName, 200);
                // fileSaver.saveCSV(lpcmBuffer, fileName);
                fileSaver.saveSpectrogram(lpcmBuffer, "output/" + fileName, 256);
            });
